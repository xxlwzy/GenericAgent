...
- Background/tmux processes do NOT inherit the injected secret env vars (`OPENAI_API_*`) if the tmux server was started before secrets were available, so a long-running frontend (e.g. `frontends/qqapp.py`) can fail with `MissingSchema: Invalid URL '/v1/chat/completions'` (empty `apibase`). Make credential resolution independent of env — e.g. cache them to a non-repo file like `~/.ga_llm.json` and have `mykey.py` fall back to it when `'://'` is missing from the base.
